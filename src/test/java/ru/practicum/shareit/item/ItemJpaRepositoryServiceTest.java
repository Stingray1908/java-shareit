        ...
        class ItemJpaRepositoryServiceTest {
            ...
            void addComment_shouldCreateCommentSuccessfully() {
                // given
                // Создаём владельца вещи и пользователя, который оставит комментарий
                Long ownerId = createTestUser();
                Long bookerId = createTestUser(); // Пользователь, который брал вещь в аренду

                // Создаём вещь
                ItemReqDTO itemDto = createItemReqDTO();
                ItemSendDTO item = itemService.create(ownerId, itemDto);
                Long itemId = item.getId();

                // Создаём бронирование, чтобы пользователь мог оставить комментарий
                createBookingForUserAndItem(bookerId, itemId, LocalDateTime.now().minusDays(2), LocalDateTime.now().minusDays(1));


                CommentReqDto commentReqDto = new CommentReqDto();
                commentReqDto.setComment("Отличный предмет, очень помог!");


                // when
                CommentSendDto result = itemService.addComment(bookerId, itemId, commentReqDto);
                System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
                System.out.println(itemService.getById(item.getId()));
                System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
                // then
                assertThat(result).isNotNull();
                assertThat(result.getComment()).isEqualTo("Отличный предмет, очень помог!");
                assertThat(result.getBookerId()).isEqualTo(bookerId);

                // Проверяем, что комментарий сохранился в БД
                List<Comment> comments = commentRepository.findByItemId(itemId);
                assertThat(comments).hasSize(1);
                Comment savedComment = comments.get(0);
                assertThat(savedComment.getComment()).isEqualTo("Отличный предмет, очень помог!");
                assertThat(savedComment.getItem().getId()).isEqualTo(itemId);
                assertThat(savedComment.getBooker().getId()).isEqualTo(bookerId);
            }*/

            private void createBookingForUserAndItem(Long userId, Long itemId, LocalDateTime start, LocalDateTime end) {
                BookingReqDto bookingDto = new BookingReqDto();
                bookingDto.setItemId(itemId);
                bookingDto.setStart(start);
                bookingDto.setEnd(end);

                bookingRepository.save(bookingJpaService.testCreate(bookingDto, userId));
            }
